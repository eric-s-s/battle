...
class GroundMapPlacementError(ValueError):
    def __init__(self, *args, **kwargs):
        super(GroundMapPlacementError, self).__init__(*args, **kwargs)
...
class GroundMap(object):
    ...
    def _raise_placement_error(self, tile: Tile):
        ...
        if not self.is_on_map(point) or self.has_tile(point):
            raise GroundMapPlacementError('Occupied or missing')
    ...
    def _raise_unit_placement_error(self, point: Point):
        if not self.can_place_unit(point):
            raise GroundMapPlacementError('illegal unit placement')
...
def _raise_too_many_tiles_error(tiles, points):
    if len(tiles) > len(points):
        raise GroundMapPlacementError('not enough points on map')
