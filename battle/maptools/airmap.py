...
class AirMap(object):
    def __init__(self, ground_elevations: Dict[Point, Union[int, float]]):
        self._all_points = sorted(ground_elevations.keys())
        self._min_elevations = {point: elevation + 1 if elevation != float('-inf') else 1
                                for point, elevation in ground_elevations.items()}

    @property
    def min_elevations(self):
        return self._min_elevations.copy()
