...
class TestMovementCalculator(unittest.TestCase):
    ...
    def test_get_movement_points_with_path_regression_test(self):
        elevations = {Point(0, 0): 0, Point(1, 0): 11, Point(2, 0): 0,  Point(3, 0): 0,
                      ...
                      Point(0, 3): 0, Point(1, 3): 0,  Point(2, 3): 0,  Point(3, 3): 0}
        tiles = [Tile(point=point, elevation=elevation * 2) for point, elevation in elevations.items()]
        map_ = Map(4, 4, tiles)
        origin = Point(0, 0)
        expected = {
            Point(0, 0): (0,  []),
        ...
        self.assertEqual(MovementCalculator(map_).get_movement_points_with_path(origin, 21), expected)
    ...
    def test_get_movement_points_with_path_regression_test_two(self):
        ...
        self.assertEqual(MovementCalculator(map_).get_movement_points_with_path(origin, 7), expected)
